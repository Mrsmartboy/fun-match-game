...
import {MatchGameContainer,MatchGameImage,TabListContainer,MatchGameItemContainer} from './styled'
...
const tabsList = [
  ...
  let intervalId;
  const MatchGame =()=>{
    const [tabListId,setTabListId]= useState(tabsList[0].tabId)
    ...
    const getRenderedList=()=>{
       const List = imagesList.filter(eachItem=>(
        eachItem.category===tabListId
       ))
    ...
    if (id===imageId){
        ...
        const randomValue =Math.ceil(Math.random()*imagesList.length)
        setImageId(imagesList[randomValue].id)
    }else{
    ...
    const getRenderedMatchItem=()=>{
        const filteredImagesList = getRenderedList()

        return (
            <MatchGameItemContainer>
                {filteredImagesList.map(eachItem=>(
                    <MatchGameItem eachItem={eachItem} key={eachItem.id} onThumbnail={onThumbnail}/>
                ))}
            </MatchGameItemContainer>
        )
    }
    ...
    const getRandomImage = ()=>{
        const data = imagesList.find(eachItem=>(
            eachItem.id===imageId
        ))
   ...
   const getRenderedMatch=()=>{
    return(
      <>
      <MatchGameImage src={randomImage} alt="image-url"/>
         <TabListContainer>
         {tabsList.map(eachTab=>(
            <TabsItem eachTab={eachTab} key={eachTab.tabId} onTab={onTab} isActive={tabListId===eachTab.tabId}/>
         ))}
         </TabListContainer>
         {getRenderedMatchItem()}
      </>
    )
   }
   ...
   const onReset=()=>{
    ...
    return(
        <>
        <NavBar timerValue={timerValue} score={score}/>
        <MatchGameContainer>
          {isScoreCard?<ScoreCard score={score} onReset={onReset}/>:getRenderedMatch()}
